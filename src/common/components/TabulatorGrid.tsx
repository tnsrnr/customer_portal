      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1">
            <button 
              className={`px-2 py-1 rounded border ${currentPage === 1 ? 'opacity-50 cursor-not-allowed' : ''} ${theme === 'dark' ? 'bg-gray-600 text-gray-200 border-gray-500 hover:bg-gray-500' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`}
            ...
            <button 
              className={`px-2 py-1 rounded border ${currentPage >= Math.ceil(dataStats.total / pageSize) ? 'opacity-50 cursor-not-allowed' : ''} ${theme === 'dark' ? 'bg-gray-600 text-gray-200 border-gray-500 hover:bg-gray-500' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`}
              onClick={() => setCurrentPage(currentPage + 1)}
              disabled={currentPage >= Math.ceil(dataStats.total / pageSize)}
            >
              Next
            </button>
            <button 
              className={`px-2 py-1 rounded border ${currentPage >= Math.ceil(dataStats.total / pageSize) ? 'opacity-50 cursor-not-allowed' : ''} ${theme === 'dark' ? 'bg-gray-600 text-gray-200 border-gray-500 hover:bg-gray-500' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`}
              onClick={() => setCurrentPage(Math.ceil(dataStats.total / pageSize))}
              disabled={currentPage >= Math.ceil(dataStats.total / pageSize)}
            >
              Last
            </button>
